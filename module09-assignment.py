...
promo_avg = sales_df[sales_df['Promotion'] == 'Yes']['Total_Sales'].mean()
no_promo_avg = sales_df[sales_df['Promotion'] == 'No']['Total_Sales'].mean()

promo_total = sales_df[sales_df['Promotion'] == 'Yes']['Total_Sales'].sum()
no_promo_total = sales_df[sales_df['Promotion'] == 'No']['Total_Sales'].sum()

promo_comparison = {
    'promo_avg_sales': promo_avg,
    'no_promo_avg_sales': no_promo_avg,
    'promo_total_revenue': promo_total,
    'no_promo_total_revenue': no_promo_total
}
...
print(f"- Promoted Items Avg Sale: ${promo_avg:,.2f}")
print(f"- Non-Promoted Items Avg Sale: ${no_promo_avg:,.2f}")
print(f"- Revenue from Promotions: ${promo_total:,.2f}")
...
print("1. Increase promotions on high-performing products like PhoneX to maximize revenue.")
print("2. Focus on regions with highest sales (e.g., North America) for expansion opportunities.")
print("3. Address missing data issues to improve accuracy of future analysis.")